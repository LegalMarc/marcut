...
def run_metadata_matrix_tests(project_root: Path):
    ...
    input_doc = project_root / ".marcut_artifacts/ignored-resources" / "sample-files" / "Sample 123 Consent.docx"
    output_dir = project_root / ".marcut_artifacts/ignored-resources" / "runs" / "metadata-matrix"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    script_path = project_root / "tests" / "scripts" / "run_metadata_matrix.py"
    
    if not input_doc.exists():
        print("⚠️ Sample DOCX missing in .marcut_artifacts/ignored-resources/sample-files; skipping metadata matrix.")
        return True

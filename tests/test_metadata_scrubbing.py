...
try:
    from marcut.docx_io import MetadataCleaningSettings, CLI_ARG_PAIRS
...
class TestMetadataCleaningSettings(unittest.TestCase):
    ...
    @unittest.skipUnless(IMPORTS_SUCCESS, "marcut.docx_io not available")
    def test_from_environment_balanced_preset(self):
        prev_preset = os.environ.get("MARCUT_METADATA_PRESET")
        prev_json = os.environ.get("MARCUT_METADATA_SETTINGS_JSON")
        try:
            os.environ["MARCUT_METADATA_PRESET"] = "balanced"
            ...
            if prev_preset is None:
                ...
                os.environ["MARCUT_METADATA_PRESET"] = prev_preset
            if prev_json is None:
                os.environ.pop("MARCUT_METADATA_SETTINGS_JSON", None)
            else:
                os.environ["MARCUT_METADATA_SETTINGS_JSON"] = prev_json
        self.assertFalse(settings.clean_headers_footers)
        self.assertFalse(settings.clean_watermarks)
        self.assertFalse(settings.clean_track_changes)

    @unittest.skipUnless(IMPORTS_SUCCESS, "marcut.docx_io not available")
    def test_from_environment_json_overrides_preset(self):
        prev_preset = os.environ.get("MARCUT_METADATA_PRESET")
        prev_json = os.environ.get("MARCUT_METADATA_SETTINGS_JSON")
        try:
            os.environ["MARCUT_METADATA_PRESET"] = "balanced"
            ...
            if prev_json is None:
                ...
                os.environ["MARCUT_METADATA_SETTINGS_JSON"] = prev_json
        self.assertTrue(settings.clean_headers_footers)
        self.assertTrue(settings.clean_watermarks)
        self.assertTrue(settings.clean_created_date)

    @unittest.skipUnless(IMPORTS_SUCCESS, "marcut.docx_io not available")
    def test_from_environment_cli_overrides_json(self):
        prev_preset = os.environ.get("MARCUT_METADATA_PRESET")
        prev_json = os.environ.get("MARCUT_METADATA_SETTINGS_JSON")
        try:
            os.environ["MARCUT_METADATA_PRESET"] = "balanced"
    ...
    @unittest.skipUnless(IMPORTS_SUCCESS, "marcut.docx_io not available")
    def test_all_fields_respected_from_json(self):
        prev_preset = os.environ.get("MARCUT_METADATA_PRESET")
        prev_json = os.environ.get("MARCUT_METADATA_SETTINGS_JSON")
        try:
            os.environ["MARCUT_METADATA_PRESET"] = "none"
            payload = {}
            expected = {}
            for idx, f in enumerate(fields(MetadataCleaningSettings())):
                value = (idx % 2) == 0
                payload[f.name] = value
                expected[f.name] = value
            os.environ["MARCUT_METADATA_SETTINGS_JSON"] = json.dumps(payload)
            settings = MetadataCleaningSettings.from_environment([])
        finally:
            if prev_preset is None:
                os.environ.pop("MARCUT_METADATA_PRESET", None)
        ...
        for field_name, expected_value in expected.items():
            self.assertEqual(getattr(settings, field_name), expected_value, msg=f"field mismatch: {field_name}")

    @unittest.skipUnless(IMPORTS_SUCCESS, "marcut.docx_io not available")
    def test_all_cli_clean_flags_enable_fields(self):
        args = [flag.replace("--no-clean-", "--clean-", 1) for flag, _ in CLI_ARG_PAIRS]
...
class TestScrubReportPrePostValues(unittest.TestCase):
    ...
    def test_preserved_field_uses_true_post_value(self):
        ...
        author_row = next(row for row in report["groups"]["Core Properties"] if row["field"] == "Author")
        self.assertEqual(author_row["before"], "Alice")
        self.assertEqual(author_row["after"], "Bob")
        self.assertEqual(author_row["status"], "observed")

    @unittest.skipUnless(IMPORTS_SUCCESS, "marcut.pipeline not available")
    def test_input_file_info_snapshot_is_respected(self):
        settings = MetadataCleaningSettings.from_preset("none")
        report = pipeline._build_scrub_report(  # pylint: disable=protected-access
            {},
            {},

...
class TestEmailPattern:
    ...
    def test_email_with_subdomain(self):
        ...
        assert len(emails) == 1
        assert 'company.co.uk' in emails[0]['text']
    
    def test_multiple_emails(self):
        """Test multiple emails in same text."""
        text = "Primary: alice@test.com, Secondary: bob@other.org"
        spans = run_rules(text)
        emails = [s for s in spans if s['label'] == 'EMAIL']
    ...
    def test_email_with_plus_sign(self):
        ...
        assert len(emails) == 1
        assert emails[0]['text'] == 'user+tag@gmail.com'
...
class TestCompanySuffixPattern:
    ...
    def test_inc_company(self):
        ...
        text = "Contract with Acme Corporation Inc."
        spans = run_rules(text)

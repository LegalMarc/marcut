...
Options:
  ...
  --input PATH        Input DOCX (default: sample-files/Sample 123 Consent.docx)
...
CONFIG_PATH = ROOT / "build-scripts" / "config.json"
...
def resolve_build_dir() -> Path:
    ...
    candidates = [
        ROOT / ".marcut_artifacts/ignored-resources" / "builds" / "build_swift",
        ROOT / "build_swift",
        ROOT / "build",
    ]
...
def resolve_latest_dmg() -> Path:
    if CONFIG_PATH.exists():
        try:
            config = json.loads(CONFIG_PATH.read_text())
            final_dmg = config.get("final_dmg")
            if final_dmg:
                candidate = (CONFIG_PATH.parent / final_dmg).resolve()
                if candidate.exists():
                    return candidate
        except Exception:
            pass

    dmg_roots = [ROOT / ".marcut_artifacts/ignored-resources", ROOT]
    for dmg_root in dmg_roots:
        dmgs = sorted(dmg_root.glob("MarcutApp-Swift-*.dmg"))
        if dmgs:
            return dmgs[-1]
    raise SystemExit('No DMG found; run scripts/sh/build_swift_only.sh first')
...
def ensure_build(skip_build: bool):
    if skip_build:
        return
    build_sh = ROOT / 'scripts' / 'sh' / 'build_swift_only.sh'
    if not build_sh.exists():
        raise SystemExit('scripts/sh/build_swift_only.sh not found')
...
def app_binary(use_dmg: bool):
    if use_dmg:
        return mount_dmg()
    app_bin = resolve_build_dir() / 'MarcutApp.app' / 'Contents' / 'MacOS' / 'MarcutApp'
    if not app_bin.exists():
        raise SystemExit('App binary not found in build_swift. Run scripts/sh/build_swift_only.sh')
    return None, app_bin
...
def make_input_docx(default_path: Path) -> Path:
    ...
    try:
        ...
        d = Document()
        d.add_paragraph('Email: sample123@example.com URL: https://example.com')
        d.save(tmp)
        return tmp
    except Exception:

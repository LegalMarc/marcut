...
inp = Path(sys.argv[1])
doc = Document()
doc.add_paragraph('Contact: test@example.com and https://example.com')
doc.save(inp)
print('ok')
PY
  then
  ...
  if /bin/bash "$LAUNCHER" -m marcut.cli redact --in "$INP" --out "$OUTDOC" --report "$OUTJSON" --backend mock --model none >/dev/null 2>&1; then
...
mount_and_check_dmg() {
  ...
  local MNT
  MNT=$(hdiutil attach -nobrowse -quiet "$DMG" | awk '{print $3}' | tail -1)

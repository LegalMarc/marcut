...
BUILD_SCRIPT="$ROOT_DIR/scripts/sh/build_swift_only.sh"
SAMPLE_DIR="$ROOT_DIR/.marcut_artifacts/ignored-resources/sample-files"
if [ ! -d "$SAMPLE_DIR" ]; then
  SAMPLE_DIR="$ROOT_DIR/sample-files"
fi
...
clean() {
  rm -rf .marcut_artifacts/ignored-files/test-output /tmp/marcut.log || true
...
mock_redaction() {
  ...
  if [ "$IS_NEW_APP" = true ]; then
    # New unified app uses Application Support sandbox
    local APP_SUPPORT_DIR="$HOME/Library/Application Support/MarcutApp"
    local INPUT_DIR="$APP_SUPPORT_DIR/Input"
    mkdir -p "$INPUT_DIR"
    INPUT_DOC="$INPUT_DIR/mock_input.docx"
    OUTPUT_DOC="$OUTDIR/mock_out.docx"
    OUTPUT_JSON="$OUTDIR/mock_report.json"

    # Copy sample file
    if [ -f "$SAMPLE_DIR/Sample 123 Consent.docx" ]; then
      cp "$SAMPLE_DIR/Sample 123 Consent.docx" "$INPUT_DOC"
    else
      # Use any available sample file
      find "$SAMPLE_DIR" -name "*.docx" | head -1 | xargs cp -t "$INPUT_DIR" || {
    ...
    if ! "$CLI_BIN" --redact --in "$INPUT_DOC" --outdir "$OUTDIR" --report "$OUTPUT_JSON" --mode rules --backend mock --model mock; then
    ...
    if [ -f "$SAMPLE_DIR/Sample 123 Consent.docx" ]; then
      ...
      find "$SAMPLE_DIR" -name "*.docx" | head -1 | xargs cp -t "$EMBEDDED_DIR" || {
        log "No sample files available, skipping mock redaction"
        return 0
      }
    fi
    ...
    if ! "$CLI_BIN" --redact --in "$INPUT_DOC" --outdir "$OUTDIR" --mode rules --backend mock --model mock; then
      err "Mock redaction failed via CLI"
      return 1
    fi
...
real_redaction_if_ollama() {
  ...
  local INPUT_DIR="$APP_SUPPORT_DIR/Input"
  mkdir -p "$INPUT_DIR"
  local INP="$INPUT_DIR/e2e_input.docx"; local OUTDOC="$OUTDIR/e2e_out.docx"; local OUTJSON="$OUTDIR/e2e_report.json"

  # Copy sample file to sandbox directory
  if [ -f "$SAMPLE_DIR/Sample 123 Consent.docx" ]; then
    cp "$SAMPLE_DIR/Sample 123 Consent.docx" "$INP"
  else
    # Create minimal test file in Work directory
    cp "$SAMPLE_DIR/*Consent*.docx" "$INP" 2>/dev/null || {
      ...
      echo "Email: sample123@example.com URL: https://legal.example" > /tmp/test_content.txt
...
cli_redaction_mock() {
  ...
  local APP_SUPPORT_DIR="$HOME/Library/Application Support/MarcutApp"
  local INPUT_DIR="$APP_SUPPORT_DIR/Input"
  local WORK_SUBDIR="$APP_SUPPORT_DIR/Work/Staging"
  local INPUT_DOC="$INPUT_DIR/cli-input.docx"
  mkdir -p "$INPUT_DIR" "$WORK_SUBDIR"
  for attr in com.apple.quarantine com.apple.provenance; do
    xattr -d "$attr" "$APP_SUPPORT_DIR" 2>/dev/null || true
  done
  ...
  if [ -f "$SAMPLE_DIR/Sample 123 Consent.docx" ]; then
    ...
    cp "$SAMPLE_DIR/"*.docx "$INPUT_DOC" 2>/dev/null || {
      # Fallback: use any docx file we can find
      find "$SAMPLE_DIR" -name "*.docx" | head -1 | xargs -I {} cp "{}" "$INPUT_DOC" 2>/dev/null || {
...
main() {
  ...
  local BAPP="$BUILD_DIR/MarcutApp.app"
  if [ "$FAST_MODE" -eq 1 ] && [ -d "$BAPP" ]; then
    log "FAST MODE: Skipping build, app bundle already exists."
  else
    timed_step "Building Swift app and DMG" bash -c "chmod +x '$BUILD_SCRIPT' && '$BUILD_SCRIPT'"
  fi
  ...
  timed_step "Mock redaction (from bundle)" mock_redaction "$BAPP" ".marcut_artifacts/ignored-files/test-output/bundle"
  ...
  local DMG; DMG=$(ls -1 "$ROOT_DIR"/.marcut_artifacts/ignored-resources/MarcutApp-Swift-*.dmg 2>/dev/null | tail -1)
  if [ -z "$DMG" ]; then
    DMG=$(ls -1 "$ROOT_DIR"/MarcutApp-Swift-*.dmg 2>/dev/null | tail -1)
  fi
  ...
  timed_step "Mock redaction (from DMG)" mock_redaction "$DAPP" ".marcut_artifacts/ignored-files/test-output/dmg"
  ...
  timed_step "Mock redaction (from installed copy)" mock_redaction "$IAPP" ".marcut_artifacts/ignored-files/test-output/installed"
  timed_step "Sandboxed CLI mock redaction" cli_redaction_mock "$IAPP"
  timed_step "Real Ollama E2E (if available)" real_redaction_if_ollama "$IAPP" ".marcut_artifacts/ignored-files/test-output/real"

  hdiutil detach "$MNT" >/dev/null 2>&1 || true
  ok "Local E2E (Full) complete. Outputs in .marcut_artifacts/ignored-files/test-output/"
}

...
class TestOutputValidator:
    ...
    def validate_app_bundle(self, app_path: Optional[Path] = None) -> Dict:
        ...
        if framework_found:
            ...
            # Test bundled Python launcher if present
            python_launcher = app_path / "Contents/Resources/run_python.sh"
            if python_launcher.exists() and os.access(python_launcher, os.X_OK):
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        validation["issues"].append(f"Bundled python test failed: {result.stderr}")
                except subprocess.TimeoutExpired:
                    validation["issues"].append("Bundled python test timeout")
                except Exception as e:
                    validation["issues"].append(f"Bundled python test error: {e}")
            else:
                validation["issues"].append("Bundled python launcher not found")
        else:

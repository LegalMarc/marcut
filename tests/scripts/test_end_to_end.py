...
class MarcutTestRunner:
    ...
    def _check_ollama_status(self) -> bool:
        ...
        try:
            ...
            print("⚠️  Ollama not available - AI pathway disabled")
            return False
    ...
    def validate_output_files(self, rules_result: Dict, ai_result: Optional[Dict]) -> Dict:
    ...
    def run_single_file_test(self, source_file: Path) -> Dict:
        ...
        # Validate outputs
        validation = self.validate_output_files(rules_result, ai_result)
        ...
        if validation["automated_checks"] == "pass":
    ...
    def generate_test_report(self) -> Dict:
        ...
        successful_rules = sum(1 for r in self.results if r.get("rules_only", {}).get("status") == "success")
        successful_ai = sum(1 for r in self.results if r.get("ai", {}).get("status") == "success")

        # Calculate averages
        rules_times = [r["rules_only"]["processing_time"] for r in self.results
                      if r.get("rules_only", {}).get("status") == "success"]
        ai_times = [r["ai"]["processing_time"] for r in self.results
                   if r.get("ai", {}).get("status") == "success"]
...
Examples:
    ...
    # Test specific file
    python3 scripts/test_end_to_end.py --file sample-files/Shareholder-Consent.docx

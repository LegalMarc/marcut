...
SAMPLE_DIR = REPO_ROOT / ".marcut_artifacts/ignored-resources" / "sample-files"
...
def valid_docx_path():
    ...
    path = SAMPLE_DIR / "Sample 123 Fee Letter (fails redaction).docx"
    if not path.exists():
...
class TestOllamaStreamingFix:
    ...
    def test_parse_llm_response_handles_markdown_wrapped_json(self):
        ...
        markdown_json = """```json
{"entities": [{"text": "Sample 123 Corp", "type": "ORG"}]}
```"""
        result = parse_llm_response(markdown_json)
        
        assert 'entities' in result
        assert result['entities'][0]['text'] == 'Sample 123 Corp'

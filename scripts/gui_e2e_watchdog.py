...
def run_one(app_bin: Path, input_path: Path, outdir: Path, mode: str, model: str, timeout_sec: int) -> bool:
    outdir.mkdir(parents=True, exist_ok=True)
    cmd = [str(app_bin), '--redact', '--in', str(input_path), '--outdir', str(outdir), '--mode', mode, '--model', model]
...
def main():
    ...
    try:
        ...
        for i in range(1, args.iterations + 1):
            ...
            # Build command line via environment only for backend override
            ok = False
            if args.backend:
                # Invoke app with backend override
                cmd = [str(app_bin), '--redact', '--in', str(input_doc), '--outdir', str(outdir), '--mode', args.mode, '--model', args.model, '--backend', args.backend]
                print('[watchdog] Launch:', ' '.join(cmd))
                start = time.time()
                proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                try:
                    ...
                    print('[watchdog] TIMEOUT — killing process')
                    kill_process_tree(proc)
                    ok = False
            else:
                ok = run_one(app_bin, input_doc, outdir, args.mode, args.model, args.timeout_sec)
            if ok:
                successes += 1
            else:
                failures += 1

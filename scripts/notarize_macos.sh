  ...
  if [ -t 0 ]; then
    ...
    if [ -z "${ASC_API_KEY_ID:-}" ] || [ -z "${ASC_API_KEY_ISSUER:-}" ] || [ -z "${ASC_API_KEY_BASE64:-}" ]; then
      ...
      if [ -z "${NOTARYTOOL_APP_PASSWORD:-}" ]; then
        ...
        echo ""
        if [ -n "${NOTARYTOOL_APP_PASSWORD:-}" ]; then
          CREDENTIALS_UPDATED=1
        fi
      fi
    fi
...
if [ -t 0 ] && [ "$CREDENTIALS_UPDATED" = "1" ]; then
...
if [ "${use_api_key}" = true ]; then
  ...
  notary_args=(--apple-id "$NOTARYTOOL_APPLE_ID" --password "$NOTARYTOOL_APP_PASSWORD")
  if [ -n "${NOTARYTOOL_TEAM_ID}" ]; then

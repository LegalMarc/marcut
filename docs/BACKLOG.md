...
- **Automated "Redaction Rationale" Reporting**: LLM-generated plain-English explanations in the audit log. The data layer landed under #68 (`MARCUT_GENERATE_RATIONALE`, off by default; `rationale.py`, `SpanRationale` in `report_schema.py`, `pipeline._annotate_missing_rationale`/`_canonicalize_cluster_rationale`/`_sanitize_cross_referenced_rationale`), and HTML report rendering (origin-appropriate badges, the persistent "AI-inferred, not verified" caveat) landed under #69 — see `docs/design/redaction_rationale_reporting.md` for the mitigation design. Both directions are now closed; no further work is planned here.

...
- **Automated "Redaction Rationale" Reporting**: LLM-generated plain-English explanations in the audit log. The data layer landed under #68 (`MARCUT_GENERATE_RATIONALE`, off by default; `rationale.py`, `SpanRationale` in `report_schema.py`, `pipeline._annotate_missing_rationale`/`_canonicalize_cluster_rationale`/`_sanitize_cross_referenced_rationale`) — see `docs/design/redaction_rationale_reporting.md` (covers how to label LLM-generated rationale as inference, not verified fact) for the still-open direction: HTML report rendering (#69).

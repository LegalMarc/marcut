...
- **Hardcoded Model Data**: Move hardcoded model tags (like `qwen2.5:14b` and their parameters) out of Swift/Python code and into a remote or local JSON config file for easier over-the-air updates.
...
- **Multi-Model Orchestration Workflow**: Use a smaller model like `phi4-mini` for a fast first pass, and route only low-confidence sentence chunks to a larger model like `qwen2.5:14b`.

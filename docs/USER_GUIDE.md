...
- If the sandbox blocks CLI file access, copy inputs into `~/Library/Group Containers/group.com.marclaw.marcutapp/MarcutOllama/Input/` (or a subfolder) before invoking the CLI.
...
- Ollama running locally (http://localhost:11434) for enhanced or balanced modes
...
- Use `--mode enhanced` for the two-pass LLM pipeline; `--mode balanced` is legacy hybrid behavior.

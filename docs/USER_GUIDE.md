...
### Batch Processing
- While processing multiple documents, the toolbar shows an **estimated time remaining** for the batch once enough documents have completed to produce a reliable estimate. Early in a run (too few samples), no estimate is shown rather than a misleading one. The estimate is **weighted by each document's word count** rather than assuming every remaining document takes as long as the ones already done, so a queue that starts with a few small/fast documents no longer produces an over-optimistic figure for the larger documents still to come.
...
- **Live progress within a document** - During AI analysis the progress bar advances continuously as the model works through each chunk, reflecting real token-level progress instead of jumping only when a whole chunk finishes. On a long document you now see steady movement rather than the bar sitting still for a minute and then leaping ahead.
- **Pre-flight checks before long runs and downloads** - Before starting a redaction run or a model download, Marcut confirms the destination folder is actually writable and that the volume has enough free disk space for the expected output (or the model being pulled). If not, you get a clear message upfront - for example "Cannot write to selected destination - please choose a different folder" or a "Not enough free disk space..." notice stating roughly how much is needed versus available - instead of the run failing partway through. (When free space genuinely can't be determined, Marcut lets the run proceed rather than blocking on a guess.)
- **Ollama port-conflict detection** - If another Ollama (or other) process is already listening on the port Marcut expects to use for its embedded AI service, Marcut detects the conflict and reports it clearly (naming the port and process) and tries a different port, rather than silently talking to - or waiting on a timeout from - a server it did not start.
- **Friendlier error messages** - When a document fails, the alert shows a short, plain-English explanation with a suggested next step (for example "The AI service isn't reachable. Make sure Ollama is running, then try again."), not a raw Python traceback. Every failure still logs the full technical detail; use **View Logs** / **Open App Log** in Settings to see it.
- **Recovery from a wedged AI worker** - If the embedded Python worker that runs the AI stops responding, Marcut now detects the stall (via an internal heartbeat) and fails the affected document with "Processing stalled - the embedded engine stopped responding. Restart Marcut to resume processing." rather than freezing the app indefinitely.
- **Sleep/wake handling** - While any document is processing, Marcut holds the Mac awake so a long run isn't interrupted by system sleep. If the machine does sleep and wake mid-run, Marcut health-checks the AI service on wake: if it's still responsive, processing continues; if it isn't, the in-flight document is failed cleanly with a message to restart Marcut, rather than hanging.

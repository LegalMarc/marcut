...
### Fix
- Consolidate the three overlapping progress mechanisms (rich `ProgressUpdate`, the five `emit_mass_event` shapes, and their raw `print()` to stdout) into one channel. `progress.py`'s `ProgressUpdate` is promoted from a `pydantic.dataclasses.dataclass` to a `BaseModel` carrying a `type: Literal["phase_update"]` discriminator, joining the five mass-event models in one `ProgressEvent` union (`validate_progress_event`/`serialize_progress_event`, renamed from `validate_mass_event`/`serialize_mass_event`); `SWIFT_HANDLED_MASS_EVENT_TYPES` is now derived from the narrower `MassEvent` union rather than hand-typed. Every producer dispatches through one call shape, `progress_callback(validated_event)` (`model_enhanced.py`'s new `_emit_progress_event` helper), with no callback-arity detection -- retiring two bugs a design review surfaced mid-flight: `IntelligentRedactionPipeline.process_document` used to build a second, redundant `ProgressTracker` around every mass event whenever `progress_callback`'s signature happened to be `inspect.signature`-readable (true for the CLI's plain callback, never true for the PythonKit bridge's `PyCFunction`), silently diverging what the CLI and the packaged app each received for the same logical event; and `LlamaCppRedactionPipeline.process_document` bypassed the schema entirely, calling `progress_callback` with a raw, unvalidated `(chunk, total, message)` 3-tuple -- it now emits a `chunk_start`/`chunk_end` pair per chunk through the same helper, deliberately without `mass_total`/`keepalive`/`token_progress` (this backend stays sparser by design). On the Swift side, the maintainer's Option B decision (posted on the issue) keeps the in-process PythonKit bridge reading the rich `phase_update`'s fields as live `PythonObject` attributes (no serialization cost); for the five mass-event types it reconstructs the same small JSON payload from the event's own attributes locally in Swift (`PythonKitBridge.swift`'s new `massEventJSONString`, not a round trip through Python's `json.dumps`) and hands it to the same `ingestProgressPayload` string parser the subprocess/stdout channel already used. `PythonBridge.swift`'s `runRedactionWithCLI`/`parseMARCUTProgress` (zero callers in the app target) are deleted. `cli.py`'s and `gui.py`'s progress callbacks now check `.type` before touching phase-only fields, since the single registered callback receives every `ProgressEvent` variant. Step 6 of the bridge-schema migration in `docs/design/bridge_schema_migration.md`, the follow-up its step 4 explicitly deferred (#96). Fix round 1: this is the hottest path in the codebase (many events per document), so a 20000-iteration microbenchmark (`_emit_progress_event` vs. a reconstruction of the old `inspect.signature`/`ProgressTracker` dispatch it replaces) measured per-event cost on both production callback shapes before landing -- PythonKit bridge shape (tracker always `None`): ~3.2us/event old vs. ~3.0-3.3us/event new (flat to slightly faster); CLI shape (old code built and updated a `ProgressTracker` per event): ~5.1-6.5us/event old vs. ~3.0us/event new (40-50% faster, since the outer per-phase tracker now owns that update instead of a second one built per mass event). Neither shape regresses. Also, fix round 1 stopped the PythonKit bridge's `keepalive` branch from surfacing `chunk`/`total` as separate typed fields on `PythonRunnerProgressUpdate`: `tracker` is always `None` on the in-process PythonKit path, so under the old 3-tuple dispatch `progress_callback(0, 0, display)` always sent `chunk=0, total=0` and a `display` string that never matched `extractChunkInfo`'s "Processing chunk N/M" regex (that phrasing required a real tracker), so the shipped app's chunk readout never advanced during keepalive ticks. Surfacing the new event's real `chunk`/`total` directly would have made that readout start advancing live where it never did before -- an undisclosed behavior change the ticket's "progress bar behaves identically" bar rules out. `keepalive` now flows through the same generic mass-event JSON-message path as `mass_total`/`chunk_start`/`chunk_end`/`token_progress`, matching its pre-#96 (silently inert) observable behavior on the packaged app. Fix round 2 (a reviewer-caught regression): round 1's `ChunkStartEvent`/`ChunkEndEvent` carried no chunk-index/total-chunks fields, so `extractChunkInfo` never matched a llama.cpp chunk boundary -- that backend has no keepalive thread and emits no `mass_total`, so its chunk_start/chunk_end pair is its only heartbeat signal, and losing it meant an aggregate-slow-but-alive llama.cpp run could be wrongly marked "processing stalled" after 120s of individually-fast chunks. Both events gain optional `chunk_index`/`total_chunks` fields, populated only by the llama.cpp emit sites; `massEventJSONString`/`massEventChunkInfo` (`PythonKitBridge.swift`) thread them into `PythonRunnerProgressUpdate.chunk`/`.total` so `extractChunkInfo`'s existing struct-field branch fires again. A new Swift test drives a simulated 5-chunk llama.cpp sequence whose aggregate elapsed time exceeds the 120s heartbeat-watchdog timeout while no single chunk does, and asserts the run is not marked stalled.

## 2026-09-10

### Fix
- Guard `PermissionManager`'s five `UNUserNotificationCenter.current()` call sites (`init`'s delegate wiring, `requestNotificationPermission`, `sendSystemNotification`, `forceRequestNotificationPermission`, `refreshNotificationStatus`) behind a new `private static let notificationCenter: UNUserNotificationCenter?` that resolves to `nil` unless `Bundle.main.bundleURL.pathExtension == "app"`. Under the `swift test` CLI runner there is no host app bundle, so `.current()` used to raise an uncaught `NSInternalInconsistencyException` ("bundleProxyForCurrentProcess is nil") and abort the entire test process -- the reason `SettingsView` (which eagerly constructs `PermissionManager.shared`) and `DocumentRedactionViewModel.processAllDocuments`/`scrubMetadataOnly` (which always call `sendSystemNotification` on completion) could not be exercised directly under test. The delegate-wiring and notification-send sites now log and no-op when `nil`; `requestNotificationPermission`/`forceRequestNotificationPermission` throw `PermissionError.notificationPermissionDenied`; `refreshNotificationStatus` reports `.notDetermined`. Inside the shipped `.app` bundle, behavior is unchanged -- the delegate is still assigned eagerly in `init`, per the existing "CRITICAL FIX" comment protecting foreground banners. Adds `testSettingsViewConstructsWithoutCrashingUnderSwiftTest` and `testProcessAllDocumentsWithNoItemsCompletesWithoutCrashing`, and rewrites the three stale test comments (`MarcutAppTests.swift`) that had documented the abort as a permanent limitation. First prerequisite (#97) for the view/view-model decomposition design spike (`docs/design/view_controller_decomposition.md`); blocks #100 and #102.
- Stop `test_metadata_zip_rewrite_peak_memory_bounded` flaking on CI measurement noise. The 1.15x bound (already raised once under #63, after CI measured 0.934x and 1.08x with the streaming fix genuinely in place) was exceeded at 1.157x on a docs-only commit that touched no source or test file, so it could not have been a regression. Rather than move the bound again -- 1.2x is where the real regression signature starts, leaving almost no headroom -- the measurement now retries. `ru_maxrss` noise only ever inflates a peak-RSS reading, never deflates it, so the minimum across attempts is the reading closest to the function's true footprint. The first measurement is taken as before and further attempts run only when it would otherwise fail, so a passing run costs the same as it did. A genuine reintroduction of whole-part buffering measures ~1.2-1.6x on every run and still fails all three; the diagnostic now reports every attempt. Verified both ways: the test passes unchanged, and against a deliberately tightened bound it fails after three attempts reporting all three readings.

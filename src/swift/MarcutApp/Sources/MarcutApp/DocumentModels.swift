...
final class DocumentItem: Identifiable, ObservableObject {
    ...
    private var heartbeatWarningIssued = false
    ...
    private func startMomentumAnimation() {
        smoothProgressTimer = Timer.scheduledTimer(withTimeInterval: 0.016, repeats: true) { [weak self] _ in
            Task { @MainActor in
    ...
    private func startMicroProgress() {
        ...
        microProgressTimer = Timer.scheduledTimer(withTimeInterval: microProgressInterval, repeats: true) { [weak self] _ in
            Task { @MainActor in
                guard let self = self else {
                    self?.microProgressTimer?.invalidate()
                    return
                }
    ...
    func markHeartbeatWarningIssued() {
        heartbeatWarningIssued = true
    }

    func heartbeatWarningAlreadyIssued() -> Bool {
        heartbeatWarningIssued
    }
    ...
    /// Check if smooth animations are currently active
    var isAnimating: Bool {
        return smoothProgressTimer != nil || microProgressTimer != nil
    }
...
enum RedactionMode: String, CaseIterable {
    ...
    var displayName: String {
        switch self {
        case .rules: return "Rules Only"
        case .enhanced: return "Enhanced (Rules + AI)"
        }
    }
    
    var description: String {
        switch self {
        case .rules: return "Fast rule-based detection for structured PII"
        case .enhanced: return "Comprehensive detection with AI validation (recommended)"
        }
    }
}
...
enum RedactionRule: String, CaseIterable, Identifiable, Codable, Hashable {
    ...
    var description: String {
        switch self {
        case .email:
            return "RFC-like email addresses: user@domain.tld (case-insensitive)."
        case .phone:
...
struct RedactionSettings {
    var mode: RedactionMode = .enhanced
    ...
    var seed: Int = 42
    var chunkTokens: Int = 1000
    var overlap: Int = 150
    var processingTimeoutSeconds: Int = 1200
...
class DebugLogger {
    ...
    private static func resolveLogDirectory() -> URL {
        ...
        if let groupBase = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.marclaw.marcutapp") {
            let dir = groupBase

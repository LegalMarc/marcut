...
struct MetadataCleaningSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                .labelsHidden()
                .accessibilityIdentifier("metadata.preset")
                    ...
                    MetadataGroupView(
                        ...
                        items: [
                            ...
                            MetadataItem(
                                ...
                                warning: "May break custom forms, bindings, or vendor integrations.",
                                binding: makeCustomBinding($settings.cleanCustomXMLParts)
                            ),
                            MetadataItem(
                                label: "Nuclear Option: Non-Standard XML Namespaces",
                                description: "Strip elements outside standard OOXML namespaces",
                                warning: "May remove embedded functionality or third-party extensions.",
                                binding: makeCustomBinding($settings.cleanNonstandardXML)
                            ),
                            MetadataItem(
                                label: "Nuclear Option: Microsoft Extension Namespaces",
                                description: "Remove Microsoft-specific extension namespaces",
                                warning: "Can remove Word-only features or compatibility metadata.",
                                binding: makeCustomBinding($settings.cleanMicrosoftExtensionXML)
                            ),
                            MetadataItem(
                                label: "Nuclear Option: Unknown Relationships",
                                description: "Drop relationships outside known schema prefixes",
                                warning: "May orphan embedded content or custom parts.",
                                binding: makeCustomBinding($settings.cleanUnknownRelationships)
                            ),
                            MetadataItem(
                                label: "Nuclear Option: Orphaned Package Parts",
                                description: "Delete unreferenced ZIP parts",
                                warning: "Removes hidden parts but can break documents relying on custom refs.",
                                binding: makeCustomBinding($settings.cleanOrphanedParts)
                            ),
                            MetadataItem(
                                label: "Nuclear Option: Alternate Content Blocks",
                                description: "Remove mc:AlternateContent and keep fallback",
                                warning: "May drop advanced features or compatibility layers.",
                                binding: makeCustomBinding($settings.cleanAlternateContent)
                            ),
                        ],
    ...
    private func selectAllInGroup(appProperties: Bool = false, coreProperties: Bool = false, documentStructure: Bool = false, embeddedContent: Bool = false, advancedHardening: Bool = false) {
        ...
        if documentStructure {
            settings.cleanReviewCommentsVisible = true
        ...
        if advancedHardening {
            ...
            settings.cleanNonstandardXML = true
            settings.cleanMicrosoftExtensionXML = true
            settings.cleanUnknownRelationships = true
            settings.cleanOrphanedParts = true
    ...
    private func invertGroup(appProperties: Bool = false, coreProperties: Bool = false, documentStructure: Bool = false, embeddedContent: Bool = false, advancedHardening: Bool = false) {
        ...
        if documentStructure {
            settings.cleanReviewCommentsVisible.toggle()
            settings.cleanReviewCommentsHidden.toggle()
        ...
        if advancedHardening {
            ...
            settings.cleanCustomXMLParts.toggle()
            settings.cleanNonstandardXML.toggle()
            settings.cleanMicrosoftExtensionXML.toggle()
            settings.cleanUnknownRelationships.toggle()
            settings.cleanOrphanedParts.toggle()
            settings.cleanAlternateContent.toggle()
        }
    }
}
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 6) {
                    ForEach(items.indices, id: \.self) { index in
                        ...
                        .accessibilityIdentifier("metadata.\(groupKey).item.\(item.label)")
                    }
                }
                .padding(.leading, 22)
            }

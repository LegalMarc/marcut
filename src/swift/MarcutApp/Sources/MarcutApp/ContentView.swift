...
extension ContentView {
    private func debugAttributeGraph() {
        ...
        print("  - @EnvironmentObject viewModel: \(ObjectIdentifier(viewModel))")
...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            if let message = viewModel.metadataReportErrorMessage {
                ReportErrorBanner(
                    title: "Metadata Report Failed",
                    message: message,
                    actionTitle: viewModel.metadataReportNeedsPermissionRetry ? "Retry File Access Permissions" : nil,
                    onAction: viewModel.metadataReportNeedsPermissionRetry
                        ? { Task { await viewModel.retryFileAccessPermissionsFromBanner() } }
                        : nil,
                    onDismiss: {
                        viewModel.clearMetadataReportError()
                    }
                )
            }
            if let message = viewModel.reportErrorMessage {
                ReportErrorBanner(title: "Report Error", message: message, onDismiss: {
        ...
        .onChange(of: viewModel.hasProcessingDocuments) { _, hasProcessing in
            if !hasProcessing {
                isStopping = false
            }
        }
    ...
    private var actionButtons: some View {
        HStack(spacing: 12) {
            ...
            Button(action: { startMetadataReport() }) {
                HStack(spacing: 8) {
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        .fill(hasMetadataReportWork ? Color.blue : Color.gray.opacity(0.4))
                        .shadow(color: hasMetadataReportWork ? Color.blue.opacity(0.2) : Color.clear, radius: 6, y: 3)
                )
            }
            .disabled(
            ...
            .animation(.easeInOut(duration: 0.2), value: hasMetadataReportWork)
            .accessibilityIdentifier("content.metadataReport")
            ...
            Button(action: { startMetadataScrub() }) {
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        .fill(hasScrubWork ? Color.orange : Color.gray.opacity(0.4))
                        .shadow(color: hasScrubWork ? Color.orange.opacity(0.2) : Color.clear, radius: 6, y: 3)
                )
            }
            .disabled(
                !hasScrubWork ||
            ...
            if isPreparing {
                ...
                .buttonStyle(.plain)
                .accessibilityIdentifier("content.stopProcessing")
                ...
                .accessibilityIdentifier("content.startProcessing")
            }
        }
    }
        ...
        Task {
            ...
            await MainActor.run {
                if outputSaveLocation.requiresPrompt {
                    ...
                    panel.directoryURL = preferredDestinationDirectory()
                    ...
                    if panel.runModal() == .OK, let destination = panel.url {
                        ...
                        currentProcessingTask = Task {
                            await viewModel.processAllDocuments(to: destination, includeRetryItems: false)
                    ...
                    currentProcessingTask = Task {
                        await viewModel.processAllDocuments(includeRetryItems: false)
                        await MainActor.run {
                            currentProcessingTask = nil
                        }
                    }
    ...
    /// Generate metadata reports without performing scrubbing.
    private func startMetadataReport() {
        ContentView.logToFile("=== METADATA REPORT BUTTON CLICKED ===")
        ContentView.logToFile("hasValidDocuments: \(viewModel.hasValidDocuments)")
        ...
        Task {
            try? await Task.sleep(nanoseconds: 50_000_000) // brief UI tick
            ...
            await MainActor.run {
                ...
                if outputSaveLocation.requiresPrompt {
                    ...
                    panel.canCreateDirectories = true
                    panel.directoryURL = preferredDestinationDirectory()
                    panel.message = "Choose a folder to save the metadata reports"
                    panel.prompt = "Save Here"
                    ...
                    if panel.runModal() == .OK, let destination = panel.url {
                        ...
                        currentProcessingTask = Task {
                            await viewModel.generateMetadataReportsInPlace(destination: destination)
                            await MainActor.run {
                                currentProcessingTask = nil
                            }
                        }
                    ...
                    currentProcessingTask = Task {
                        await viewModel.generateMetadataReportsInPlace()
                        await MainActor.run {
                            currentProcessingTask = nil
                        }
                    }
    ...
    private func retryDocument(_ item: DocumentItem) {
        guard !item.status.isProcessing else { return }
        ContentView.logToFile("=== RETRY DOCUMENT CLICKED ===")
            ...
            await MainActor.run {
                ...
                if !item.ensureFileAccess() {
                    alertInfo = AlertInfo(
                        ...
                        message: "The document is not accessible. Re-add the file and try again."
                    )
                    isPreparing = false
                    return
                }
                ...
                if outputSaveLocation.requiresPrompt {
                    ...
                    panel.canChooseDirectories = true
                    panel.canCreateDirectories = true
                    panel.directoryURL = preferredDestinationDirectory(for: item)
                    panel.message = operation == .scrub
                        ? "Choose a folder to save the metadata-scrubbed documents"
                        : "Choose a folder to save the retried redaction"
                    panel.prompt = "Save Here"
                    ...
                    if panel.runModal() == .OK, let destination = panel.url {
                        ...
                        viewModel.retryDocument(item, destination: destination, operation: operation)
                    }
                } else {
                    isPreparing = false
                    viewModel.retryDocument(item, destination: nil, operation: operation)
                }
            }
        }
    }
        ...
        Task {
            ...
            await MainActor.run {
                if outputSaveLocation.requiresPrompt {
                    ...
                    currentProcessingTask = Task {
                        await viewModel.scrubMetadataOnly()
                        await MainActor.run {
                            currentProcessingTask = nil
                        }
                    }
    ...
    private func finishDropLoad() {
        pendingDropLoads = max(pendingDropLoads - 1, 0)
        if pendingDropLoads == 0 {
            Task { await viewModel.refreshEnvironmentStatus() }
        }
    }

    private func rememberExplicitDestination(_ destination: URL) {
        let standardized = destination.standardizedFileURL
        lastExplicitOutputDirectoryPath = standardized.path
    }

    private func lastExplicitDestinationDirectory() -> URL? {
        let trimmed = lastExplicitOutputDirectoryPath.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func primaryDocumentDirectory(preferred item: DocumentItem? = nil) -> URL? {
        if let item {
            return item.url.deletingLastPathComponent()
        }
        return viewModel.items.first?.url.deletingLastPathComponent()
    }

    private func preferredDestinationDirectory(for item: DocumentItem? = nil) -> URL? {
        if let explicit = lastExplicitDestinationDirectory() {
            return explicit
        }
        switch outputSaveLocation {
        case .sameAsOriginal:
            return primaryDocumentDirectory(preferred: item)
        case .alwaysAsk:
            return primaryDocumentDirectory(preferred: item)
    ...
    private var hasReadyDocuments: Bool {
        viewModel.items.contains { item in
            if item.status == .validDocument || item.status.canRetry {
                return true
            }
            if item.status.isComplete {
                return item.reportOutputURL == nil && item.reportHTMLOutputURL == nil
            }
            return false
        }
    }

    private var hasMetadataReportWork: Bool {
        viewModel.items.contains { item in
...
struct DocumentListView: View {
    ...
    var body: some View {
                ...
                ForEach(viewModel.items) { item in
                    Group {
                        if item.status == .failed || item.status == .invalidDocument {
                            DocumentRow(item: item, viewModel: viewModel, alertInfo: $alertInfo, onRetry: onRetry)
        ...
        .onReceive(timer) { _ in
            for item in viewModel.items where item.status.isProcessing {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if let message = item.metadataReportErrorMessage, !item.status.isProcessing {
                    HStack(spacing: 8) {
                        Text(message)
                            .font(.system(size: 11))
                            .foregroundColor(CustomColors.destructiveColor(for: colorScheme))
                            .lineLimit(2)
                        if item.metadataReportNeedsPermissionRetry {
                            Button("Retry File Access") {
                                Task { await viewModel.retryFileAccessPermissions(for: item) }
                            }
            ...
            if item.status.isComplete {
                ...
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .trailing, spacing: 6) {
                        HStack(spacing: 8) {
                            ...
                            isEnabled: metadataReportAvailable,
                        ...
                        TooltipButton(
                            ...
                            isEnabled: true,
                            accessibilityId: "document.saveMetadata.\(item.id.uuidString)"
                        )
                    }
                }
                ...
                HStack(spacing: 8) {
                    ...
                    if item.status == .failed || item.status == .cancelled {
                        ...
                        .buttonStyle(.bordered)
                        .controlSize(.small)
                        .accessibilityIdentifier("document.retry.\(item.id.uuidString)")
                    }
                }
            }
        }
...
struct ReportErrorBanner: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if let actionTitle, let onAction {
                    Button(actionTitle, action: onAction)
                        .font(.system(size: 12, weight: .semibold))
                        .buttonStyle(.link)
                        .accessibilityIdentifier("content.metadataReport.retryPermissions")
                }
            }
            Spacer()
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("content.metadataReport.dismiss")
        }
        .padding(.horizontal, 16)
        .padding(.vertical, 10)
        .background(
            RoundedRectangle(cornerRadius: 10)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 10)
                        .stroke(CustomColors.destructiveColor(for: colorScheme).opacity(0.25), lineWidth: 1)
                )
        )
    }
}

...
struct MarkdownTextView: NSViewRepresentable {
    ...
    private func updateContent(textView: NSTextView) -> [String: Int] {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if line.hasPrefix("# ") {
                ...
                let processed = processInlineMarkdown(
                    bulletContent,
            ...
            for match in matches {
                ...
                if nsFullRange.location + nsFullRange.length <= result.length {
                    result.replaceCharacters(
                        in: nsFullRange,
                        with: NSAttributedString(string: boldText, attributes: boldAttrs)
                    )
        ...
        if let regex = try? NSRegularExpression(pattern: codePattern) {
            ...
            for match in matches {
                ...
                if nsFullRange.location + nsFullRange.length <= result.length {
                    result.replaceCharacters(
                        in: nsFullRange,
                        with: NSAttributedString(string: codeText, attributes: codeAttrs)
                    )
    ...
    class Coordinator: NSObject, NSTextViewDelegate {
        ...
        func scrollToAnchor(_ anchorID: String) {
            guard let textView,
                  let position = anchorPositions[anchorID]
            else {
                return
            }

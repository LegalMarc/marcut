...
- [Technical Reference (Developer)](#technical-reference-developer)
  ...
  - [Sandboxed Paths (Application Support)](#sandboxed-paths-application-support)
...
If you are new to Marcut, start with the macOS app and Rules + AI (Enhanced) mode. If you are automating, skip to the CLI Guide.
...
2. Ensure the AI service is available (skip if using Rules only):
...
| Rules + AI (Rules Override) | Default accuracy with guardrails | Yes | AI adds spans; deterministic rules remain. CLI alias: `enhanced` or `rules_override`. |
| Rules + AI (Constrained LLM Overrides) | Reduce false positives in ORG/NAME/LOC | Yes | LLM can drop only ORG/NAME/LOC rule spans at high confidence (default 0.99 in the app; CLI default 0.95). |
| Rules + AI (LLM Overrides) | Aggressive AI cleanup | Yes | LLM can drop rule spans across labels; use with caution. |

### Preflight Checklist

- Input is a real `.docx` (not `.doc`, `.pdf`, or a renamed file).
- Output paths are writable (CLI creates parent dirs for `--out` and `--report`).
- Any Rules + AI mode: model is installed and the backend is ready.
  - Ollama (app): embedded service should start automatically and bind to `127.0.0.1`.
  - Ollama (CLI from source): ensure `OLLAMA_HOST` points to your local service.
...
2. Run MarcutApp in Rules + AI mode with the default model.
...
- EMAIL: email addresses, for example `sample123@example.com`.
...
- SWIFT: SWIFT/BIC codes (8 or 11 characters).
...
- ORG: company names with legal suffixes, for example `Sample 123 LLC`.
...
- Sandboxed app builds resolve this inside the app container; use Settings > Reveal Models to open the exact location.
- Model downloads do not prompt for a save location.
...
- Normal Mode (default): Rules + AI uses Rules Override at 99% confidence (temp 0.0, chunk 1400, overlap 200).
...
Rules Engine
- Toggle deterministic rules: EMAIL, PHONE, SSN, MONEY, NUMBER, DATE, ACCOUNT, SWIFT, CARD, URL, IP, ORG, LOC, Signature Names, IMAGES.
...
- Temperature (0.0 to 2.0, default 0.0): lower is more deterministic.
- Chunk Size (500 to 2000 tokens, default 1400): larger uses more context but can be slower.
- Chunk Overlap (50 to 500 tokens, default 200): higher reduces boundary misses but costs more.
- Processing Timeout (30s to 195 min, default 120 min, ∞ for no limit): max time for the LLM processing phase.
- Random Seed (1 to 1000, default 42): helps reproducibility.
- Edit System Prompt: affects AI extraction prompts (Ollama and llama.cpp); validation prompts are fixed.
...
- `--seed <int>` (default: 42).
- `--llm-skip-confidence <float>` (default: 0.95): confidence required to allow a SKIP decision (higher = more likely to redact).
...
# Rules + AI with llama.cpp
marcut redact --in input.docx --out runs/out.docx --report runs/out_report.json \
  --mode enhanced --backend llama_cpp --llama-gguf /path/to/model.gguf --threads 8
...
Core runtime
- `OLLAMA_HOST`: local host:port for Ollama (CLI and Python pipeline). Host is forced to `127.0.0.1`.
- `MARCUT_OLLAMA_HOST`: host:port used by the macOS app (set alongside `OLLAMA_HOST`, loopback enforced).
...
- `MARCUT_SYSTEM_PROMPT_PATH`: path to the AI extraction system prompt (Ollama and llama.cpp).
...
- Matching is case-insensitive (literals and regex).
- Leading determiners (the/a/an/etc.) are ignored for matching.
- Simple plurals are treated as equivalent for literals (trailing `s`, `(s)`, or `ies` -> `y`).
- Exclusions apply to deterministic rules and AI validation.
- In Rules Only and guardrailed Rules + AI modes (Rules Override, Constrained Overrides), ORG/NAME/LOC spans made up only of excluded words and connectors are skipped.

Example file:
```
# Do not redact these terms
Sample 123 Corporation
Project Titan
\bFY2024\b
```

App locations
- `~/Library/Application Support/MarcutApp/Overrides/` (sandboxed builds resolve this inside the app container; use the in-app editor to open it).
...
- The override applies to AI extraction prompts for both Ollama and llama.cpp.
- Deterministic rules and validation prompts are not affected.
...
4. Run AI extraction (Rules + AI modes only).
...
- Highest: EMAIL, PHONE, SSN, CARD, ACCOUNT, SWIFT, URL, IP
...
Excluded terms and regex patterns apply to ORG, NAME, and LOC matches.

### AI Extraction and Validation

Rules + AI modes use a multi-pass pipeline:
- Rule-based detection for structured PII.
- LLM extraction for names, organizations, locations, and context-sensitive entities.
- Selective validation for uncertain spans using surrounding context (batch validation when possible).
- Consistency pass that rescans for exact matches of confirmed entities.

Override behaviors:
- Rules Override: AI adds spans; deterministic rules remain.
...
- `llm_skip_confidence`: threshold for allowing the AI to drop rule spans in override modes.
...
- `settings`: snapshot of run settings (mode, mode_requested, backend, model, chunk_tokens, overlap, temperature, seed, llm_skip_confidence, rule_filter, rules).
...
### Sandboxed Paths (Application Support)

Common paths used by the app:
- Models: `~/Library/Application Support/MarcutApp/models/`
- Overrides: `~/Library/Application Support/MarcutApp/Overrides/`
- Logs: `~/Library/Application Support/MarcutApp/logs/`
- Staging: `~/Library/Application Support/MarcutApp/Work/Staging/`

Sandboxed builds resolve these inside the app container; use Reveal buttons in Settings to open the active directories.
...
- The only network activity is for model downloads; inference uses a local Ollama server bound to `127.0.0.1`.
- Remote Ollama hosts are not supported; only the port can change.
...
macOS app (GUI / PythonKit)
- Reads inputs in place via security-scoped access.
- Validation copies the DOCX to a temp directory for integrity checks, then securely deletes it.
- Outputs are written to the selected destination.

macOS app CLI (`MarcutApp --redact`)
- Inputs must live under the app support container.
- Files are copied into `Work/Staging` for sandbox-safe processing.
- Staging artifacts are removed after each run.

Python package CLI (`marcut redact`)
...
- Access to Documents or user-selected folders for importing and saving files.
...
- "AI service is not available" or "Ollama not running"
  - macOS app: relaunch the app (embedded service starts automatically).
  - CLI from source: start your local Ollama service (`ollama serve`).
  - Check `OLLAMA_HOST` if you use a custom local port.

- "AI model is not available"
  - macOS app: download the model in Manage Models.
  - CLI from source: `ollama pull <model>`.
...
- Confirm the rule is enabled (for deterministic rules).
  - Use Rules + AI (enhanced) mode to add AI extraction.
...
A: `~/Library/Application Support/MarcutApp/models/` (sandboxed builds resolve this inside the app container; use Reveal Models).
...
A: No. It only affects deterministic rules; AI extraction still runs in Rules + AI modes.

Q: Can I customize what the AI looks for?
A: You can adjust mode, model, temperature, exclusions, and (optionally) the extraction system prompt. The override affects AI extraction prompts; validation prompts are fixed.

Q: Why are some names missed in Rules + AI?
A: Small models, low chunk overlap, or excluded terms can still miss edge cases. Increase model size, overlap, or review exclusions.
...
- Rules + AI (Enhanced): Rules plus LLM extraction and validation.
- Rules Override: Rules + AI with guardrails; AI adds spans, deterministic rules remain.
- Constrained LLM Overrides: AI can drop ORG/NAME/LOC rule spans at high confidence.
- LLM Overrides: AI can drop rule spans across labels; most aggressive mode.
...
- MarcutApp bundles a local Python runtime and third-party libraries. License files are included inside the app bundle.
- Ollama and model licenses are managed by their respective providers.

### Open Source Licenses

License file base path:

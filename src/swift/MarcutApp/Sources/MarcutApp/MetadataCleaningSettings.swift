...
enum MetadataCleaningPreset: String, CaseIterable, Identifiable {
    ...
    var description: String {
        switch self {
        case .maximum: "Clears all metadata fields for maximum privacy"
        case .balanced: "Clears identifying info while preserving document formatting"
        case .none: "No metadata cleaning (keep all original metadata)"
        case .custom: "Use your custom selection below"
        }
    }
}
...
struct MetadataCleaningSettings: Codable, Equatable {
    ...
    var cleanReviewCommentsVisible: Bool = true
    var cleanReviewCommentsHidden: Bool = true
    ...
    // MARK: - Nuclear Options (Non-Standard / Hidden Data)

    var cleanCustomXMLParts: Bool = true
    var cleanNonstandardXML: Bool = true
    var cleanMicrosoftExtensionXML: Bool = true
    var cleanUnknownRelationships: Bool = true
    var cleanOrphanedParts: Bool = true
    var cleanAlternateContent: Bool = true
    ...
    static var maximumPrivacy: MetadataCleaningSettings {
        ...
        // All fields ON for maximum privacy
    ...
    static var balanced: MetadataCleaningSettings {
        ...
        settings.cleanAltText = true // Accessibility
        ...
        settings.cleanSharedDoc = true // Document state
        ...
        settings.cleanHiddenText = true // Hidden content
        settings.cleanInvisibleObjects = true // Hidden objects
        settings.cleanCustomXMLParts = false // Nuclear options off
        settings.cleanNonstandardXML = false
        settings.cleanMicrosoftExtensionXML = false
        settings.cleanUnknownRelationships = false
        settings.cleanOrphanedParts = false
        settings.cleanAlternateContent = false

        // Comments & track changes defaults
        settings.cleanReviewCommentsVisible = false
        settings.cleanReviewCommentsHidden = true
        ...
        // Track changes - OFF by default; hidden review comments removed
    ...
    static var none: MetadataCleaningSettings {
        ...
        settings.cleanReviewCommentsVisible = false
        settings.cleanReviewCommentsHidden = false
        settings.cleanTrackChanges = false
        ...
        settings.cleanCustomXMLParts = false
        settings.cleanNonstandardXML = false
        settings.cleanMicrosoftExtensionXML = false
        settings.cleanUnknownRelationships = false
        settings.cleanOrphanedParts = false
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case cleanReviewCommentsVisible
        case cleanReviewCommentsHidden
        ...
        case cleanActiveX
        case cleanCustomXMLParts
        case cleanNonstandardXML
        case cleanMicrosoftExtensionXML
        case cleanUnknownRelationships
        case cleanOrphanedParts
        case cleanAlternateContent
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let defaults = MetadataCleaningSettings.balanced
        ...
        cleanTotalEditingTime = try container.decodeIfPresent(Bool.self, forKey: .cleanTotalEditingTime) ?? defaults
            .cleanTotalEditingTime
        cleanApplication = try container.decodeIfPresent(Bool.self, forKey: .cleanApplication) ?? defaults
            .cleanApplication
        ...
        cleanHyperlinkBase = try container.decodeIfPresent(Bool.self, forKey: .cleanHyperlinkBase) ?? defaults
            .cleanHyperlinkBase
        cleanStatistics = try container.decodeIfPresent(Bool.self, forKey: .cleanStatistics) ?? defaults.cleanStatistics
        cleanDocSecurity = try container.decodeIfPresent(Bool.self, forKey: .cleanDocSecurity) ?? defaults
            .cleanDocSecurity
        cleanScaleCrop = try container.decodeIfPresent(Bool.self, forKey: .cleanScaleCrop) ?? defaults.cleanScaleCrop
        cleanLinksUpToDate = try container.decodeIfPresent(Bool.self, forKey: .cleanLinksUpToDate) ?? defaults
            .cleanLinksUpToDate
        cleanSharedDoc = try container.decodeIfPresent(Bool.self, forKey: .cleanSharedDoc) ?? defaults.cleanSharedDoc
        cleanHyperlinksChanged = try container.decodeIfPresent(Bool.self, forKey: .cleanHyperlinksChanged) ?? defaults
            .cleanHyperlinksChanged

        cleanAuthor = try container.decodeIfPresent(Bool.self, forKey: .cleanAuthor) ?? defaults.cleanAuthor
        cleanLastModifiedBy = try container.decodeIfPresent(Bool.self, forKey: .cleanLastModifiedBy) ?? defaults
            .cleanLastModifiedBy
        ...
        cleanContentStatus = try container.decodeIfPresent(Bool.self, forKey: .cleanContentStatus) ?? defaults
            .cleanContentStatus
        cleanCreatedDate = try container.decodeIfPresent(Bool.self, forKey: .cleanCreatedDate) ?? defaults
            .cleanCreatedDate
        cleanModifiedDate = try container.decodeIfPresent(Bool.self, forKey: .cleanModifiedDate) ?? defaults
            .cleanModifiedDate
        cleanLastPrinted = try container.decodeIfPresent(Bool.self, forKey: .cleanLastPrinted) ?? defaults
            .cleanLastPrinted
        cleanRevisionNumber = try container.decodeIfPresent(Bool.self, forKey: .cleanRevisionNumber) ?? defaults
            .cleanRevisionNumber
        ...
        cleanCustomProperties = try container.decodeIfPresent(Bool.self, forKey: .cleanCustomProperties) ?? defaults
            .cleanCustomProperties

        let legacyReviewComments = try container.decodeIfPresent(Bool.self, forKey: .cleanReviewComments)
        cleanReviewCommentsVisible = try container.decodeIfPresent(Bool.self, forKey: .cleanReviewCommentsVisible)
            ?? legacyReviewComments
            ?? defaults.cleanReviewCommentsVisible
        cleanReviewCommentsHidden = try container.decodeIfPresent(Bool.self, forKey: .cleanReviewCommentsHidden)
            ?? legacyReviewComments
            ?? defaults.cleanReviewCommentsHidden
        cleanTrackChanges = try container.decodeIfPresent(Bool.self, forKey: .cleanTrackChanges) ?? defaults
            .cleanTrackChanges
        cleanRSIDs = try container.decodeIfPresent(Bool.self, forKey: .cleanRSIDs) ?? defaults.cleanRSIDs
        cleanDocumentGUID = try container.decodeIfPresent(Bool.self, forKey: .cleanDocumentGUID) ?? defaults
            .cleanDocumentGUID
        cleanSpellGrammarState = try container.decodeIfPresent(Bool.self, forKey: .cleanSpellGrammarState) ?? defaults
            .cleanSpellGrammarState
        cleanDocumentVariables = try container.decodeIfPresent(Bool.self, forKey: .cleanDocumentVariables) ?? defaults
            .cleanDocumentVariables
        cleanMailMerge = try container.decodeIfPresent(Bool.self, forKey: .cleanMailMerge) ?? defaults.cleanMailMerge
        cleanDataBindings = try container.decodeIfPresent(Bool.self, forKey: .cleanDataBindings) ?? defaults
            .cleanDataBindings
        cleanDocumentVersions = try container.decodeIfPresent(Bool.self, forKey: .cleanDocumentVersions) ?? defaults
            .cleanDocumentVersions
        cleanInkAnnotations = try container.decodeIfPresent(Bool.self, forKey: .cleanInkAnnotations) ?? defaults
            .cleanInkAnnotations
        cleanHiddenText = try container.decodeIfPresent(Bool.self, forKey: .cleanHiddenText) ?? defaults.cleanHiddenText
        cleanInvisibleObjects = try container.decodeIfPresent(Bool.self, forKey: .cleanInvisibleObjects) ?? defaults
            .cleanInvisibleObjects
        cleanHeadersFooters = try container.decodeIfPresent(Bool.self, forKey: .cleanHeadersFooters) ?? defaults
            .cleanHeadersFooters
        ...
        cleanHyperlinkURLs = try container.decodeIfPresent(Bool.self, forKey: .cleanHyperlinkURLs) ?? defaults
            .cleanHyperlinkURLs
        ...
        cleanDigitalSignatures = try container.decodeIfPresent(Bool.self, forKey: .cleanDigitalSignatures) ?? defaults
            .cleanDigitalSignatures
        cleanPrinterSettings = try container.decodeIfPresent(Bool.self, forKey: .cleanPrinterSettings) ?? defaults
            .cleanPrinterSettings
        cleanEmbeddedFonts = try container.decodeIfPresent(Bool.self, forKey: .cleanEmbeddedFonts) ?? defaults
            .cleanEmbeddedFonts
        cleanGlossary = try container.decodeIfPresent(Bool.self, forKey: .cleanGlossary) ?? defaults.cleanGlossary
        cleanFastSaveData = try container.decodeIfPresent(Bool.self, forKey: .cleanFastSaveData) ?? defaults
            .cleanFastSaveData

        cleanExternalLinks = try container.decodeIfPresent(Bool.self, forKey: .cleanExternalLinks) ?? defaults
            .cleanExternalLinks
        ...
        cleanInternalURLs = try container.decodeIfPresent(Bool.self, forKey: .cleanInternalURLs) ?? defaults
            .cleanInternalURLs
        ...
        cleanChartLabels = try container.decodeIfPresent(Bool.self, forKey: .cleanChartLabels) ?? defaults
            .cleanChartLabels
        cleanFormDefaults = try container.decodeIfPresent(Bool.self, forKey: .cleanFormDefaults) ?? defaults
            .cleanFormDefaults
        cleanLanguageSettings = try container.decodeIfPresent(Bool.self, forKey: .cleanLanguageSettings) ?? defaults
            .cleanLanguageSettings
        cleanActiveX = try container.decodeIfPresent(Bool.self, forKey: .cleanActiveX) ?? defaults.cleanActiveX
        cleanCustomXMLParts = try container.decodeIfPresent(Bool.self, forKey: .cleanCustomXMLParts) ?? defaults
            .cleanCustomXMLParts
        cleanNonstandardXML = try container.decodeIfPresent(Bool.self, forKey: .cleanNonstandardXML) ?? defaults
            .cleanNonstandardXML
        cleanMicrosoftExtensionXML = try container
            .decodeIfPresent(Bool.self, forKey: .cleanMicrosoftExtensionXML) ?? defaults.cleanMicrosoftExtensionXML
        cleanUnknownRelationships = try container
            .decodeIfPresent(Bool.self, forKey: .cleanUnknownRelationships) ?? defaults.cleanUnknownRelationships
        cleanOrphanedParts = try container.decodeIfPresent(Bool.self, forKey: .cleanOrphanedParts) ?? defaults
            .cleanOrphanedParts
        cleanAlternateContent = try container.decodeIfPresent(Bool.self, forKey: .cleanAlternateContent) ?? defaults
            .cleanAlternateContent
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(cleanReviewCommentsVisible, forKey: .cleanReviewCommentsVisible)
        try container.encode(cleanReviewCommentsHidden, forKey: .cleanReviewCommentsHidden)
        ...
        try container.encode(cleanCustomXMLParts, forKey: .cleanCustomXMLParts)
        try container.encode(cleanNonstandardXML, forKey: .cleanNonstandardXML)
        try container.encode(cleanMicrosoftExtensionXML, forKey: .cleanMicrosoftExtensionXML)
        try container.encode(cleanUnknownRelationships, forKey: .cleanUnknownRelationships)
        try container.encode(cleanOrphanedParts, forKey: .cleanOrphanedParts)
        try container.encode(cleanAlternateContent, forKey: .cleanAlternateContent)
    }
    ...
    func toCLIArguments() -> [String] {
        ...
        if !cleanCompany {
            args.append("--no-clean-company")
        }
        if !cleanManager {
            args.append("--no-clean-manager")
        }
        if !cleanTotalEditingTime {
            args.append("--no-clean-editing-time")
        }
        if !cleanApplication {
            args.append("--no-clean-application")
        }
        if !cleanAppVersion {
            args.append("--no-clean-app-version")
        }
        if !cleanTemplate {
            args.append("--no-clean-template")
        }
        if !cleanHyperlinkBase {
            args.append("--no-clean-hyperlink-base")
        }
        if !cleanStatistics {
            args.append("--no-clean-statistics")
        }
        if !cleanDocSecurity {
            args.append("--no-clean-doc-security")
        }
        if !cleanAuthor {
            args.append("--no-clean-author")
        }
        if !cleanLastModifiedBy {
            args.append("--no-clean-last-modified-by")
        }
        if !cleanTitle {
            args.append("--no-clean-title")
        }
        if !cleanSubject {
            args.append("--no-clean-subject")
        }
        if !cleanKeywords {
            args.append("--no-clean-keywords")
        }
        if !cleanComments {
            args.append("--no-clean-comments")
        }
        if !cleanCategory {
            args.append("--no-clean-category")
        }
        if !cleanContentStatus {
            args.append("--no-clean-content-status")
        }
        if !cleanCreatedDate {
            args.append("--no-clean-created-date")
        }
        if !cleanModifiedDate {
            args.append("--no-clean-modified-date")
        }
        if !cleanLastPrinted {
            args.append("--no-clean-last-printed")
        }
        if !cleanRevisionNumber {
            args.append("--no-clean-revision")
        }
        if !cleanIdentifier {
            args.append("--no-clean-identifier")
        }
        if !cleanLanguage {
            args.append("--no-clean-language")
        }
        if !cleanVersion {
            args.append("--no-clean-version")
        }
        if !cleanCustomProperties {
            args.append("--no-clean-custom-props")
        }
        if !cleanReviewCommentsVisible {
            args.append("--no-clean-review-comments-visible")
        }
        if !cleanReviewCommentsHidden {
            args.append("--no-clean-review-comments-hidden")
        }
        if !cleanTrackChanges {
            args.append("--no-clean-track-changes")
        }
        if !cleanRSIDs {
            args.append("--no-clean-rsids")
        }
        if !cleanDocumentGUID {
            args.append("--no-clean-guid")
        }
        if !cleanMailMerge {
            args.append("--no-clean-mail-merge")
        }
        if !cleanDataBindings {
            args.append("--no-clean-data-bindings")
        }
        if !cleanDocumentVersions {
            args.append("--no-clean-doc-versions")
        }
        if !cleanInkAnnotations {
            args.append("--no-clean-ink-annotations")
        }
        if !cleanHiddenText {
            args.append("--no-clean-hidden-text")
        }
        if !cleanInvisibleObjects {
            args.append("--no-clean-invisible-objects")
        }
        if !cleanHeadersFooters {
            args.append("--no-clean-headers-footers")
        }
        if !cleanWatermarks {
            args.append("--no-clean-watermarks")
        }
        if !cleanThumbnail {
            args.append("--no-clean-thumbnail")
        }
        if !cleanHyperlinkURLs {
            args.append("--no-clean-hyperlinks")
        }
        if !cleanAltText {
            args.append("--no-clean-alt-text")
        }
        if !cleanOLEObjects {
            args.append("--no-clean-ole")
        }
        if !cleanVBAMacros {
            args.append("--no-clean-macros")
        }
        if !cleanDigitalSignatures {
            args.append("--no-clean-signatures")
        }
        if !cleanPrinterSettings {
            args.append("--no-clean-printer")
        }
        if !cleanEmbeddedFonts {
            args.append("--no-clean-fonts")
        }
        if !cleanGlossary {
            args.append("--no-clean-glossary")
        }
        if !cleanFastSaveData {
            args.append("--no-clean-fast-save")
        }

        // Advanced hardening
        if !cleanExternalLinks {
            args.append("--no-clean-ext-links")
        }
        if !cleanUNCPaths {
            args.append("--no-clean-unc-paths")
        }
        if !cleanUserPaths {
            args.append("--no-clean-user-paths")
        }
        if !cleanInternalURLs {
            args.append("--no-clean-internal-urls")
        }
        if !cleanOLESources {
            args.append("--no-clean-ole-sources")
        }
        if !cleanImageEXIF {
            args.append("--no-clean-exif")
        }
        if !cleanStyleNames {
            args.append("--no-clean-style-names")
        }
        if !cleanChartLabels {
            args.append("--no-clean-chart-labels")
        }
        if !cleanFormDefaults {
            args.append("--no-clean-form-defaults")
        }
        if !cleanLanguageSettings {
            args.append("--no-clean-language-settings")
        }
        if !cleanActiveX {
            args.append("--no-clean-activex")
        }
        if !cleanCustomXMLParts {
            args.append("--no-clean-custom-xml-parts")
        }
        if !cleanNonstandardXML {
            args.append("--no-clean-nonstandard-xml")
        }
        if !cleanMicrosoftExtensionXML {
            args.append("--no-clean-microsoft-extensions")
        }
        if !cleanUnknownRelationships {
            args.append("--no-clean-unknown-rels")
        }
        if !cleanOrphanedParts {
            args.append("--no-clean-orphaned-parts")
        }
        if !cleanAlternateContent {
            args.append("--no-clean-alternate-content")
        }

        // Hidden settings (App Props / structure)
        if !cleanScaleCrop {
            args.append("--no-clean-scale-crop")
        }
        if !cleanLinksUpToDate {
            args.append("--no-clean-links-up-to-date")
        }
        if !cleanSharedDoc {
            args.append("--no-clean-shared-doc")
        }
        if !cleanHyperlinksChanged {
            args.append("--no-clean-hyperlinks-changed")
        }
        if !cleanSpellGrammarState {
            args.append("--no-clean-spell-grammar")
        }
        if !cleanDocumentVariables {
            args.append("--no-clean-doc-vars")
        }

        return args
    }

    /// Full settings payload for Python environment transport.
    func toEnvironmentJSON() -> String? {

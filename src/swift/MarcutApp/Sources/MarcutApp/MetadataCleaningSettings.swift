...
struct MetadataCleaningSettings: Codable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        cleanCustomXMLParts = try container.decodeIfPresent(Bool.self, forKey: .cleanCustomXMLParts) ?? defaults.cleanCustomXMLParts
        cleanNonstandardXML = try container.decodeIfPresent(Bool.self, forKey: .cleanNonstandardXML) ?? defaults.cleanNonstandardXML
        cleanMicrosoftExtensionXML = try container.decodeIfPresent(Bool.self, forKey: .cleanMicrosoftExtensionXML) ?? defaults.cleanMicrosoftExtensionXML
        cleanUnknownRelationships = try container.decodeIfPresent(Bool.self, forKey: .cleanUnknownRelationships) ?? defaults.cleanUnknownRelationships
        cleanOrphanedParts = try container.decodeIfPresent(Bool.self, forKey: .cleanOrphanedParts) ?? defaults.cleanOrphanedParts
        cleanAlternateContent = try container.decodeIfPresent(Bool.self, forKey: .cleanAlternateContent) ?? defaults.cleanAlternateContent
    }
    ...
    func toCLIArguments() -> [String] {
        ...
        if !cleanReviewCommentsVisible { args.append("--no-clean-review-comments-visible") }
        if !cleanReviewCommentsHidden { args.append("--no-clean-review-comments-hidden") }
        ...
        if !cleanCustomXMLParts { args.append("--no-clean-custom-xml-parts") }
        if !cleanNonstandardXML { args.append("--no-clean-nonstandard-xml") }
        if !cleanMicrosoftExtensionXML { args.append("--no-clean-microsoft-extensions") }
        if !cleanUnknownRelationships { args.append("--no-clean-unknown-rels") }
        if !cleanOrphanedParts { args.append("--no-clean-orphaned-parts") }
        if !cleanAlternateContent { args.append("--no-clean-alternate-content") }

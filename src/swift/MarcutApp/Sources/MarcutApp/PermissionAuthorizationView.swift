...
struct PermissionAuthorizationView: View {
    ...
    var body: some View {
        VStack(spacing: 30) {
            ...
            // Processing State
            if permissionManager.isAuthorizing {
                VStack(spacing: 12) {
                    ProgressView()
                        .scaleEffect(1.2)

                    Text(permissionManager.authorizationStep)
                        .font(.subheadline)
                        .foregroundColor(.secondary)
            ...
            // Error Display
            if let error = permissionManager.authorizationError {
                VStack(spacing: 8) {
            ...
            // Action Buttons
            if !permissionManager.isAuthorizing && permissionManager.authorizationError == nil {
                VStack(spacing: 16) {
                    Button(action: {
                        Task {
                            await permissionManager.requestAllPermissions()
                    ...
                    .buttonStyle(.borderedProminent)
                    .controlSize(.large)
                    .disabled(permissionManager.isAuthorizing)
        ...
        .onAppear {
            ...
            permissionManager.authorizationError = nil
        }
    }
}

...
# CLI/settings configuration surface, extracted verbatim to
# marcut/docx_pkg/settings.py (docx_io package split, Slice 1 -- see
# docs/design/docx_io_package_split.md). Re-exported here so existing
# `from .docx_io import ...` call sites keep working unchanged.
...
# _rewrite_docx_zip()'s raw ZIP/XML post-processing pass, extracted to
# marcut/docx_pkg/zip_postprocess.py (docx_io package split, Slice 3 -- see
# docs/design/docx_io_package_split.md). `DocxMap._rewrite_docx_zip` below
# is kept as a thin delegating method so existing call sites are unaffected.
from .docx_pkg.zip_postprocess import rewrite_docx_zip as _rewrite_docx_zip_impl

# Document scanning/indexing layer -- _iter_part_elements,
# _iter_part_elements_with_parts, the _scan_* family, and _build() --
# extracted to marcut/docx_pkg/scan.py (docx_io package split, Slice 4 --
# see docs/design/docx_io_package_split.md). `DocxMap` composes a
# `DocumentIndex` in `__init__` and re-exposes `.text`/`.index`/
# `.detached_parts` below for backward compatibility.
from .docx_pkg.scan import DocumentIndex
...
class DocxMap:
    ...
    def _iter_part_elements(self) -> Iterable[Any]:
        ...
        `DocumentIndex`. Kept as a thin delegating `DocxMap` method since
        the hardening/revision-writing code below (Slice 5) still calls
        `self._iter_part_elements()` directly.
        """

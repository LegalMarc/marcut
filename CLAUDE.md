...
- **docx_io.py** - Thin backward-compatibility shim re-exporting the full `marcut.docx_pkg` public surface (`docs/design/docx_io_package_split.md`, #76); the docx_io package split is complete, all logic now lives under `docx_pkg/`
...
- **docx_pkg/scan.py** - Document scanning/indexing layer (`DocumentIndex`): walks body/headers/footers/footnotes/endnotes/text-boxes/content-controls into the `.text`/`.index` character-offset span index `apply_replacements()` depends on; split out of `docx_io.py`'s `_iter_part_elements`/`_scan_*` family/`_build()`, composed by `DocxMap` in `__init__` and re-exposed as `.text`/`.index`/`.detached_parts` for backward compatibility (`docs/design/docx_io_package_split.md`, #75)
...
- **model_config.py** - Loader for the shared `models.json` model catalog; also the leaf home of the shared llama.cpp dispatch predicate (`uses_llama_cpp_backend()`, built on `is_gguf_model_path()`), the single definition every dispatch and display site calls (`pipeline.py` re-exports it as `_uses_llama_cpp_backend` and uses it in `_sanitize_model_for_report`, `model_enhanced.py` and `unified_redactor.py` import it directly) instead of re-deriving the check (#87)
